...
from controllers.folio import FolioController
from models.folio import DocumentoHipoteca

class TestFolioController(TestCase):

    def test_create_hipoteca_success(self):
        ...
        controller = FolioController(body, None)
        controller.create_hipoteca = MagicMock(
            return_value={
                "statusCode": 201,
                "body": "mocked HTML",
            },
        ...
        # Verificación de la llamada a la función y resultado
        controller.create_hipoteca.assert_called()
        self.assertEqual(response['statusCode'], 201)
        self.assertEqual(response['body'], 'mocked HTML')


    def test_create_hipoteca_integration(self):
        # Configuración de la simulación
        body = {
                    "apoderado": {
                        "nombre": "LUZ MARIA DURAN RODRIGUEZ",
        ...
        controller = DocumentoHipoteca(body, None)
        response = controller.create_hipoteca()
        ...
        # Podrías realizar verificaciones adicionales aquí, como:
        # - Verificar que ciertos elementos estén presentes en el HTML
        # - Comparar el HTML generado con un HTML esperado (si tienes uno)
